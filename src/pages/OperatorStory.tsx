...
import { AlertTriangle, BookOpen, ChevronLeft, Shield, Target, User, Users, Zap } from 'lucide-react';
...
import { useGetProgressQuery, useGetSceneQuery } from '@/api/storyApi';
...
const OperatorStory = () => {
  ...
  const charactersData = charactersQuery?.data;
  const storyProgress = progressQuery?.data;
  const currentSceneId = storyProgress?.currentSceneId ?? 0;
  const sceneQuery = useGetSceneQuery(currentSceneId);
  const currentScene = sceneQuery?.data;

  const allCharacters = charactersData || [];
  const character = allCharacters.find((candidate) => candidate.id === id) || selectedOperator || allCharacters[0] as Operator;
  ...
  const currentSceneLabel = currentScene ? `Scene ${currentScene.sceneNumber}` : 'Scene unavailable';
  const currentChapterLabel = currentScene ? `Chapter ${currentScene.chapterId}` : 'Chapter unavailable';
  const lastChoiceEntry = storyProgress?.choicesMade ? Object.entries(storyProgress.choicesMade).at(-1) : null;
  const continueStoryHref = currentSceneId
    ? `/story/operator/${character.id}/scene/${currentSceneId}`
    : `/story/operator/${character.id}/scene/1`;
            ...
            >
              ...
              <CyberCard className="p-6 bg-gradient-to-r from-primary/5 to-secondary/5 border-primary/20">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-xs text-muted-foreground">
                    ...
                    <span className="font-mono">{currentSceneLabel}</span>
                  </div>
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>Last Choice</span>
                    <span className="font-mono">
                      {lastChoiceEntry ? `Scene ${lastChoiceEntry[0]} -> Choice ${lastChoiceEntry[1]}` : 'No prior decisions'}
                    </span>
                  </div>
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>Progress</span>
                    <span className="font-mono">{currentChapterLabel}</span>
                  </div>
                </div>
                <CyberButton variant="hero" className="w-full mt-4" asChild>
                  <Link to={continueStoryHref}>
                    Continue POV - {currentSceneLabel}
                  </Link>
                </CyberButton>
              </CyberCard>
              ...
              <CyberCard className="p-6 bg-secondary/5 border-secondary/20">
                ...
                <div className="w-full bg-secondary rounded-full h-2">
                  <div className="bg-gradient-to-r from-primary to-secondary h-2 rounded-full" style={{ width: `${Math.min(trustLevel, 100)}%` }} />
                </div>
            ...
            >
              ...
              <div className="grid gap-1 bg-border/20 rounded-xl p-1">
                {[
                  ...
                  <CyberButton
                    ...
                    onClick={() => setActiveTab(tabId as 'overview' | 'backstory' | 'dossier' | 'missions')}
                  >
              ...
              <div className="space-y-6 mt-6">
                ...
                {activeTab === 'missions' && (
                  <div>
                    ...
                    <CyberCard className="p-6 bg-secondary/5 border-secondary/20">
                      <p className="text-sm text-foreground/80 leading-relaxed mb-4">
                        Trust modifiers from this operator influence squad dynamics. Choices here unlock or alter mission behaviors, faction tension, and accusation outcomes.
                      </p>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <CyberButton variant="hero" size="lg" className="w-full" asChild>
                  <Link to={continueStoryHref}>
                    <span>Continue <span className="font-mono uppercase tracking-wider">Story</span> {currentSceneLabel}</span>
                  </Link>
                </CyberButton>
